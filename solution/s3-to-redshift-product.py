...
sc1 = SparkContext()
glueContext1 = GlueContext(sc)
spark1 = glueContext.spark_session
...
## @return: <output>
## @inputs: []
datasource_products = glueContext1.create_dynamic_frame.from_catalog(database = "glue_db", table_name = "redshift_db_sales_stg_products", redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasource_products")
## @type: DataSource
...
## @return: <output>
## @inputs: []
datasource_category = glueContext1.create_dynamic_frame.from_catalog(database = "glue_db", table_name = "redshift_db_sales_tgt_dim_category", redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasource_category")
## @type: RenameField
## @args: [old_name = "<old_name>", new_name = "<new_name>", transformation_ctx = "<transformation_ctx>"]
## @return: <output>
## @inputs: [frame = <frame>]
rename_01 = RenameField.apply(frame = datasource_category, old_name = "category", new_name = "c_category", transformation_ctx = "rename_01")

## @type: Join
## @args: [keys1 = [<keys1>], keys2 = [<keys2>]]
## @return: <output>
## @inputs: [frame1 = <frame1>, frame2 = <frame2>]
join_1 = Join.apply(frame1 = datasource_products, frame2 = rename_01, keys1 = "category", keys2 = "category_id", transformation_ctx = "join_1")
## @type: SelectFields
...
datasink_7 = glueContext1.write_dynamic_frame.from_catalog(frame = resolvechoice_6, database = "glue_db", table_name = "redshift_db_sales_tgt_dim_products", redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_7")
job1.commit()
